...
pub fn part01(grid: &Vec<Vec<i32>>, trailheads: &Vec<(usize, usize)>, width: usize, height: usize) -> usize {
    let mut result = 0;

    for trail in trailheads {
        let mut set: HashSet<(usize, usize)> = HashSet::new();
        find_hikes(grid, &mut set, *trail, 0, width, height);
        result += set.len();
    }
    return result;
}
    ...
    if elevation == 9 {
        ...
        for point in points {
            if grid[point.1][point.0] == elevation + 1 {
                find_hikes(grid, set, point, elevation + 1, width, height);
            }
        }
        return 0;
    }
